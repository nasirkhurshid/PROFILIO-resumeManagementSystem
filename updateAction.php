<?php           //almost same as action.php (Sorry for bad documentation!)
    session_start();
    ...
    if(isset($_SESSION['user'])){
        ...
        try{
            ...
            $rec = $stmt->fetch();
            $todel = $rec['path'];
            unlink($todel);     //deleting old image from files

            $pdo = new PDO($conn,DB_U,DB_P);        //deleting old record
            $del = "DELETE FROM basics WHERE id=:id";
            $stmt = $pdo->prepare($del);
            $stmt->bindValue(':id', $id);
            $stmt->execute();

            $path = "images/{$id}-profile_$img_name";       //then inserting new
            $pdo = new PDO($conn,DB_U,DB_P);
            ...
            $pdo = new PDO($conn,DB_U,DB_P);
            $del = "DELETE FROM skills WHERE id=:id";
            $stmt = $pdo->prepare($del);
            $stmt->bindValue(':id', $id);
            $stmt->execute();
            ...
            $pdo = new PDO($conn,DB_U,DB_P);
            $del = "DELETE FROM languages WHERE id=:id";
            $stmt = $pdo->prepare($del);
            $stmt->bindValue(':id', $id);
            $stmt->execute();
            ...
            $pdo = new PDO($conn,DB_U,DB_P);
            $del = "DELETE FROM hobbies WHERE id=:id";
            $stmt = $pdo->prepare($del);
            $stmt->bindValue(':id', $id);
            $stmt->execute();
            ...
            $pdo = new PDO($conn,DB_U,DB_P);
            $del = "DELETE FROM reference WHERE id=:id";
            $stmt = $pdo->prepare($del);
            $stmt->bindValue(':id', $id);
            $stmt->execute();
